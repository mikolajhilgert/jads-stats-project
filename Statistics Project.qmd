...
```{r}
twitch_data$Mature <- as.logical(twitch_data$Mature)
twitch_data$Partnered <- as.logical(twitch_data$Partnered)
model <- glm(Partnered ~ Mature + Language + Watch.time.hours + Followers + Followers.delta + Views.gained + Partnered, data = twitch_data, family = binomial())
summary(model)
```
